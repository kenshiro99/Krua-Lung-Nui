...
function renderCashierView() {
  ...
  if (currentCashierTab === "dinein") {
    ...
    tableListContainer.innerHTML = window.posState.tables.map(t => {
      ...
      const tableOrders = dineinOrders.filter(o => o.tableId === t.id || o.tableName === t.name);
      ...
      return `
        <div class="cashier-table-item ${isOccupied ? 'occupied' : 'available'} ${isSelected ? 'active' : ''}" onclick="selectCashierTarget('${t.id}', 'dinein')">
          <div>
            ...
            <div style="font-size:0.75rem; color:var(--text-muted);">${t.seats || 4} ที่นั่ง</div>
          </div>
...
function renderCashierBillDetails() {
  ...
  if (currentCashierTab === "dinein") {
    ...
    if (!table) return;
    unpaidOrders = window.posState.orders.filter(o => (o.tableId === table.id || o.tableName === table.name) && o.paymentStatus === "unpaid");
...
function confirmPaymentAndCloseTable() {
  if (currentCashierTab === "dinein") {
    ...
    window.posState.orders.forEach(o => {
      if ((o.tableId === table.id || o.tableName === table.name) && o.paymentStatus === "unpaid") {
    ...
    alert(`✅ เช็คบิลโต๊ะ ${table.name} เรียบร้อยแล้ว!`);
  } else {
    const order = window.posState.orders.find(o => o.id === selectedCashierTableId);
    if (order) {
      order.paymentStatus = "paid";
      order.status = "completed";
      alert(`✅ รับชำระเงินคิวสั่งกลับบ้าน ${order.tableName} เรียบร้อยแล้ว!`);
    }
  }
